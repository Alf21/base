...
public class TestCommands
{
	...
	public boolean pickup(Player p)
	{
		...
		location.y += 10;
		SampObjectManager.Companion.get().createPickup(351, 15, location);
		return true;
	}
	...
	{
		Menu menu = SampObjectManager.Companion.get().createMenu("test1", 1, new Vector2D(0, 0), 100, 100);
	...
	{
		Radius location = new Radius(p.getLocation(), 10.0f);
		location.x += 10;

		p.setCheckpoint(Checkpoint.create(location, (Player player) -> {
